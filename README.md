...
「どう解決するか」：<br>
- 投稿時にアレルギー品目（食品表示法の28品目）の申告を必須にし、さらに管理者承認を通過したレシピのみを公開する2段階の仕組みにより、信頼性のある情報のみが検索対象となる
- 保護者が自身のレシピを気軽に投稿・共有できるユーザー投稿型の設計により、実体験に基づくレシピが継続的に増える
- コメント機能により、同じアレルギーに悩む保護者同士が代替食材や調理の工夫を共有できるコミュニティを形成する

## 今後の展開
- 通知機能（いいねやコメント通知、新着レシピ通知など）を導入し、ユーザーの継続利用を促す
...
### レシピ投稿の承認フロー
信頼性のあるアレルギー情報のみを公開するため、レシピ投稿は以下のフローを経て公開される。

```mermaid
flowchart LR
	A[ユーザーが投稿申請] --> B{管理者がチェック}
	B -->|承認| C[全ユーザーに公開]
	B -->|差戻し| D[編集画面に戻り再申請]
	D --> A
```
